...
CREATE TABLE IF NOT EXISTS `popcorndb`.`Tickets` (
  `TicketPrKey` INT NOT NULL AUTO_INCREMENT,
  `TicketID` VARCHAR(36) NOT NULL,
  `HallClassId` INT NOT NULL,
  `ScheduleID` INT NULL,
  PRIMARY KEY (`TicketPrKey`),
...
CREATE TABLE IF NOT EXISTS `popcorndb`.`TicketRows` (
  ...
  `TicketPrKey` INT NOT NULL,
  `SeatID` INT NOT NULL,
  PRIMARY KEY (`RowID`),
  INDEX `SeatID_idx` (`SeatID` ASC) VISIBLE,
  INDEX `TicketID_idx` (`TicketPrKey` ASC) VISIBLE,
  ...
  CONSTRAINT `Row-TicketID`
    FOREIGN KEY (`TicketPrKey`)
    REFERENCES `popcorndb`.`Tickets` (`TicketPrKey`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
